        ...
        {queue.length === 0 ? (
          ...
          <div className="flex flex-col gap-1.5">
            {queue.map((task, i) => (
              <div
                key={`${task.name}-${i}`}
                className="font-mono text-[10px] font-medium px-3 py-1.5 rounded animate-queue-slide bg-neon-purple/10 text-neon-purple border-l-2 border-l-neon-purple truncate"
                title={task.name}
              >
