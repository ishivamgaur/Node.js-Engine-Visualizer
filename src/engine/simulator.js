...
function generateTimeline(code) {
  ...
  // Track mocked memory context for TDZ/Hoisting visualization
  const executionContexts = [{
    id: 1, name: 'Global Execution Context', type: 'global', memory: {}, closures: []
  }];

  // Pre-process variables for Hoisting and TDZ (Temporal Dead Zone)
  for (const op of operations) {
    if (op.type === 'VARIABLE_DECLARATION') {
      // var gets hoisted as undefined, let/const get hoisted as <uninitialized> (TDZ)
      executionContexts[0].memory[op.name] = {
        type: op.kind || 'var',
        value: (op.kind === 'var' ? undefined : '<uninitialized>')
      };
    }
  }
  ...
  function snapshot() {
    return {
      ...
      executionContexts: JSON.parse(JSON.stringify(executionContexts)), // Deep copy contexts
    };
  }
  ...
  function handleSyncOp(op) {
    if (op.type === 'CONSOLE') {
      ...
      // Transition from TDZ (<uninitialized>) to initialized value
      if (executionContexts[0].memory[op.name].value === '<uninitialized>') {
        addStep('TDZ_RESOLUTION', `Variable '${op.name}' initialized, escaping TDZ`, 'memory', 'execution', op.line);
      }
      executionContexts[0].memory[op.name].value = '<initialized>';
