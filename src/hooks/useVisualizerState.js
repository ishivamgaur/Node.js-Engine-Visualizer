...
console.log('End');`);
  ...
  const getInitialMode = () => {
    ...
    if (tab === 'analysis' || tab === 'code') return 'code';
    if (['multi-request', 'js-execution', 'browser-render'].includes(tab)) {
      return tab;
    }
    return 'code'; // Default to Analysis (code)
  };
  ...
  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    const tabValue = mode === 'code' ? 'analysis' : mode;
